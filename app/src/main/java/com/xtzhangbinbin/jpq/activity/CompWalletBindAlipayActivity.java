...
public class CompWalletBindAlipayActivity extends BaseActivity {
    ...
    public void init(){
        ...
        comp_wallet_bind_submit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(StringUtil.isEmpty(comp_wallet_bind_alipay.getText().toString())){
                    ToastUtil.show(getApplicationContext(), "请输入支付宝帐号");
                } else {
                    final OrdinaryDialog dialog = OrdinaryDialog.newInstance(CompWalletBindAlipayActivity.this).setMessage1("绑定支付宝帐号").setMessage2("您确定要绑定该帐号吗？").showDialog();
    ...
    private void submit() {
        ...
        map.put("card_alipay_id", comp_wallet_bind_alipay.getText().toString());
        ...
        OKhttptils.post(CompWalletBindAlipayActivity.this, Config.COMP_WALLET_BIND_ALIPAY, map, new OKhttptils.HttpCallBack() {
            ...
            public void fail(String response) {
                ...
                try {
                    ...
                    Toast.makeText(CompWalletBindAlipayActivity.this, obj.getString("message"), Toast.LENGTH_SHORT).show();
