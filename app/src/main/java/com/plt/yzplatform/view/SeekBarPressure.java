...
public class SeekBarPressure extends View {
    ...
    private static int mThumbMarginTop = 30;   //滑动块顶部离view顶部的距离
    private static int mTextViewMarginTop = 15;   //当前滑块文字距离view顶部距离
    ...
    public int maxSize = 100;      //最大值
    public int maxCount = 0;
    public boolean isInside = false; //是否吸入节点
    public int endPoint;//终点位置
    public int startIndex;//起点位置
    public int space;//间距
    ...
    public SeekBarPressure(Context context, AttributeSet attrs, int defStyle) {
        ...
        notScrollBarBg = getResources().getDrawable(R.drawable.base_bg);
        hasScrollBarBg = getResources().getDrawable(R.drawable.select_bg);
        mThumbLow = getResources().getDrawable(R.drawable.seekbarpressure_thumb);
        mThumbHigh = getResources().getDrawable(R.drawable.seekbarpressure_thumb);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        startIndex = formatInt(defaultScreenLow / 100 * (mDistance)) + mThumbWidth / 2;
        endPoint = formatInt(defaultScreenHigh / 100 * (mDistance)) + mThumbWidth / 2;
        space = endPoint / maxCount;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //当前滑块刻度
        double progressLow = formatInt((mOffsetLow - mThumbWidth / 2) * maxSize / mDistance);
        double progressHigh = formatInt((mOffsetHigh - mThumbWidth / 2) * maxSize / mDistance);
...
//        canvas.drawText((int) progressHigh + "", (int) mOffsetHigh - 2, mTextViewMarginTop, text_Paint);
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (e.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            mThumbHigh.setState(STATE_NORMAL);
            if (isInside) {
                double lowSpace = (mOffsetLow - startIndex) / space;
                BigDecimal lowBD = new BigDecimal(new Double(lowSpace).toString());
                int lowIndex = lowBD.setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
                if(lowIndex >= maxCount - 2){
                    mOffsetLow = startIndex + ((maxCount - 2) * space);
                } else {
                    mOffsetLow = startIndex + (lowIndex * space);
                }

                double hiSpace = (mOffsetHigh - startIndex) / space;
                BigDecimal hiBD = new BigDecimal(new Double(hiSpace).toString());
                int hiIndex = hiBD.setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
                if(hiIndex > maxCount - 2){
                    mOffsetHigh = endPoint;
                } else {
                    mOffsetHigh = startIndex + (hiIndex * space);
                }

//                if(maxLength / mOffsetHigh < 1.3){
//                    mOffsetHigh = formatInt(defaultScreenHigh / 100 * (mDistance)) + mThumbWidth / 2;
//                    invalidate();
//                } else {
//                    double high = mOffsetHigh / 1.05 / (maxLength / maxCount-1);
//                    BigDecimal highbd = new BigDecimal(new Double(high).toString());
//                    int highIndex = highbd.setScale(0, BigDecimal.ROUND_HALF_UP).intValue();
//                    mOffsetHigh = (highIndex + 0.25) * (maxLength / maxCount);
//                    invalidate();
//                }
                invalidate();
            }
